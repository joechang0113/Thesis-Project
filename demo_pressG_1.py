...
def main(yolo):
    ...
    if writeVideo_flag:
        ...
        out = cv2.VideoWriter('output_test_cross0614_G.avi', fourcc, 15,
                              (w, h))
    ...
    while True:
        ...
        one_second_ago = now + datetime.timedelta(seconds=-1.0)
        ...
        for track in tracker.tracks:
            ...
            # if (now-one_second_ago).total_seconds() == 1.0:
            if tracker_Id in trackId_list and keyboard.is_pressed(
                    'g'):  # if key 'g' was pressed
                ...
                with open('./output_process_0614_G.csv', 'a') as csvfile:
                ...
                with open('./output_XYBegin_0614_G.csv', 'w') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(
                        ['XYBegin',
                         globals()['trackerId' + str(tracker_Id)]])
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            # Save file to excel with csv in output
            with open('./output_test_cross0614_G.csv', 'w') as csvfile:
                for i in trackId_list:
                    writer = csv.writer(csvfile)
                    writer.writerow([
                        'trackerId' + str(i),
