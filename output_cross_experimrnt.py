...
###受測者資料取出###############################################################
df = pd.read_csv("output_test_cross0510.csv", header=None)
...
for i in (0,row_number):
    ...
    #print(trackid_xy)
    #print(trackid_name)
    globals()[str(trackid_name)]=list(eval(trackid_xy)) #建立各trackerid的list  #轉str為list **important step
    num=len(globals()[str(trackid_name)])-1
    delete_list=[]
    for a in range(0,num):
        if a!=num:
            if abs(globals()[str(trackid_name)][a][0]-globals()[str(trackid_name)][a+1][0])<=2 and abs(globals()[str(trackid_name)][a][1]-globals()[str(trackid_name)][a+1][1])<=2:
...
for a in range(0,number):
    if a!=number:
        if abs(org_xy[a][0]-org_xy[a+1][0])<=2 and abs(org_xy[a][1]-org_xy[a+1][1])<=2:
            #print(org_xy[a])
